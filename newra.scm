...
(define-immutable-record-type <ra>
  (make-ra* data base dims vref vset! vlen) ra?
  (data ra-data)
  (base ra-base)
  (dims ra-dims)
  (vref ra-vref)
...
(define (pick-typed-vector-functions v)
  (cond ((vector? v)    (values vector-ref    vector-set!                     vector-length))
        ((c64vector? v) (values c64vector-ref c64vector-set!                  c64vector-length))
        ...
        ((bitvector? v) (values bitvector-ref bitvector-set!                  bitvector-length))
...
(define (make-ra data base dims)
  (receive (vref vset! vlen) (pick-typed-vector-functions data)
    (make-ra* data base dims vref vset! vlen)))
...
(define (ra-ref ra)
  (if (and (ra? ra) (null? (ra-dims ra)))
    ((ra-vref ra) (ra-data ra) (ra-base ra))
    ra))

(define (ra-set! ra rb)
  (if (null? (ra-dims ra))
    ((ra-vset ra) (ra-data ra) (ra-base ra) (ra-ref rb))
    (throw 'bad-rank-ra-set! ra rb)))
...
(define (ra-cell ra . i)
  (ra-ref (fold (lambda (i ra) (ra-item* ra i)) ra i)))
...
(define (ra-for-each op ra0 . ra)
  (apply ra-for-each-cell-naive (ra-rank ra0) (lambda ra (apply op (map ra-cell ra))) ra0 ra))
